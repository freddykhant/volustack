...
VoluStack is the **intelligent operating system for self-coached athletes** — a platform that
...
- **Not a nutrition/food logger.** TDEE is training *context*, not calorie tracking.
...
explains decisions in natural language. Any message that would *change the plan* round-trips
...
The model never returns a plan. It returns a *call to the engine* with parsed intent; the
...
chart where each muscle is colored against *its own* landmarks:
...
The user always *approves the coach*; reasoning is one tap away, never in the face.
...
Engine + fractional-attribution library is the *initial* moat (correct where others guess) →
